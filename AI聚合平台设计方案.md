...
**目标**：把平台从“功能骨架”升级为清晰的后台产品，普通前台只消费平台配置，后台负责管理平台能力。

**交付内容**：
- 前台不展示外观设置、主题切换器和原型编号类文案。
- 全站主题由后台系统设置统一配置，普通用户不可自行切换。
- 保留内部主题 Key 以降低迁移风险，对外显示为“极简风”“现代风”。
- 后台新增系统设置入口，集中管理站点名称、副标题、默认主题、注册开关、默认角色和 API Key 默认过期天数。

**验收标准**：
- 首页、登录页、列表页、详情页、API Key 页、开发者页和后台页均读取同一平台配置。
- 前台菜单和右上角不出现“外观”“原型6”“原型7”。
- 后台配置主题后刷新全站生效。

### 大阶段 2：用户管理产品化

**目标**：让管理员能在后台完成用户生命周期管理，而不是只修改已有用户状态。

**交付内容**：
- 后台用户管理支持新增用户、编辑邮箱/显示名/状态、重置密码、分配角色。
- 默认关闭公开注册，用户主要由管理员后台创建。
- 用户列表支持搜索、状态筛选、分页和操作反馈。
- 用户新增、编辑、禁用、启用、重置密码、角色变更均写入审计日志。

**验收标准**：
- 管理员可在 UI 中新增用户并用该用户登录。
- 非管理员无法调用后台用户管理接口。
- 公开注册关闭时 `/api/v1/auth/register` 返回明确拒绝。

### 大阶段 3：导航聚合落地

**目标**：把跳转链接从“后台资源项”升级为首页导航聚合能力。

**交付内容**：
- `redirect_links` 增加分类和排序字段。
- 公开链接接口只返回 `ACTIVE` 链接，并按分类、排序、ID 稳定排序。
- 首页新增“导航聚合”模块，按分类展示链接卡片，点击后新窗口打开。
- 后台跳转链接管理支持名称、URL、分类、排序、图标、描述和状态。

**验收标准**：
- 后台新增或下架链接后，首页导航聚合随接口数据变化。
- 下架链接不出现在前台。
- 链接多时能够按分类组织，而不是平铺混乱展示。

### 大阶段 4：资源管理体验补齐

**目标**：把后台资源页从简单 CRUD 表格提升为可日常使用的管理界面。

**交付内容**：
- 用户、Agents、Skills、分类、模型、数据集、微调任务、跳转链接等后台表格统一提供搜索、状态筛选和分页。
- 创建/编辑表单具备必填校验、提交加载态、错误提示和成功反馈。
- 删除、禁用、重置密码等危险操作必须二次确认。
- 后台系统设置、资源管理和审计查询统一继承全站主题。

**验收标准**：
- 管理员能通过后台完成核心资源维护。
- 表单错误不会静默失败。
- `npm run build` 和关键浏览器流程验证通过。

### 大阶段 5：验证与交付闭环

**目标**：确保 V3 能通过自动化测试、容器启动和浏览器验收。

**交付内容**：
- 后端测试覆盖平台配置、注册开关、后台用户新增/编辑、链接过滤排序。
- 前端构建验证主题文案、路由和页面类型。
- Docker 启动脚本验证完整系统可运行。
- README 同步后台设置、导航聚合、全站主题和用户新增说明。

**验收标准**：
- `mvn test` 通过。
- `npm run build` 通过。
- `docker compose config --quiet` 通过。
- `scripts/start.ps1` 可构建并启动。
- 浏览器可验证首页导航聚合、后台新增用户和后台主题配置全站生效。
